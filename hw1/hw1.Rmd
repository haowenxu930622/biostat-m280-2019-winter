...
```{bash eval=FALSE}
 awk -F ',' '{ print $2 }' /home/m280data/NYCHVS/NYCHVS_1991.csv | sort | uniq -c | sort -nr
```
| $Value$ | $Count$ |
|-----|----------|
| 1 | 2271 |
...
The first command remove everything in test1.txt file and write'hello, world' into the same file.
The second command append 'hello, world' into the last line of test2.txt file.

3. **Solution:**

The output print the last 5 lines of the first 20 lines in the 'pride_and_prejudice.txt' file. "$1", "$2", and "$3" are corresponding to "pride_and_prejudice.txt", "20" and "5" in our argument.  The first line tells us that the script should be executed using 'sh' shell.
